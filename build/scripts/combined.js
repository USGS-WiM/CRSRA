...
require([
  ...
  map = new Map("mapDiv", {
    basemap: "gray",
    center: crsra.globals.mapCenter,
  ...
  //esriConfig.defaults.geometryService = new esri.tasks.GeometryService("http://gis.wim.usgs.gov/arcgis/rest/services/Utilities/Geometry/GeometryServer");
  esriConfig.defaults.geometryService = new GeometryService(
    "https://gis1.wim.usgs.gov/server/rest/services/Utilities/Geometry/GeometryServer"
  );
  ...
  function refreshBookmarks() {
    if (useLocalStorage) {
      ...
      var appBMs = [];
      array.forEach(crsra.bookmarks, function (bm) {
        if (bm.userCreated === false) {
          appBMs.push(bm.id);
        }
      });
      var bmStorageArray = crsra.bookmarks.slice();
      ...
      cookie(storageName, dojo.toJson(crsra.bookmarks), {
        expires: exp,
      });
    }
  }
  ...
  function removeUserBookmarks() {
    ...
    //removes user bookmarks from the crsra.bookmarks array
    for (var i = 0; i < crsra.bookmarks.length; i++) {
      var obj = crsra.bookmarks[i];

      if (userBMs.indexOf(obj.id) !== -1) {
        crsra.bookmarks.splice(i, 1);
        i--;
        //!!!IMPORTANT:If adding another permanent bookmark (non-user defined) may need another i decrement.
      }
    }
  ...
  function showShareModal() {
    ...
    var cleanURL = "https://glcwra.wim.usgs.gov/crsra/";
  ...
  function printMap() {
    ...
    if (userTitle === "") {
      template.layoutOptions = {
        ...
        copyrightText:
          "This page was produced by the CRSRA web application at glcwra.wim.usgs.gov/crsra",
    ...
    var printMap = new PrintTask(
      "https://gis1.wim.usgs.gov/arcgis/rest/services/Utilities/PrintingTools/GPServer/Export%20Web%20Map%20Task"
    );
  ...
  function saveUserBookmark() {
    ...
    if (userBookmarkTitle.length > 0) {
      ...
      crsra.bookmarks.push(currentMapExtentJSON);
  ...
  $(document).ready(function () {
    ...
    crsra.bookmarks.forEach(function (bm) {
      if (bm.userCreated === false) {
        var bookmarkButton = $(
          '<tr id="' +
            bm.id +
    ...
    $("body").on("click", ".td-bm", function () {
      ...
      crsra.bookmarks.forEach(function (bookmark) {
  ...
  require([
    ...
    const dikesLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "CRSRA/MapServer",
      { id: "dikes", visible: false, minScale: 20000000 }
    );
    ...
    const degFlowlinesLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "CRSRA/MapServer",
      { id: "degFlowlines", visible: false, minScale: 2000000 }
    );
    ...
    const culvertsLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "CRSRA/MapServer",
      { id: "culverts", visible: false, minScale: 2000000 }
    );
    ...
    zonalStatsGP = new Geoprocessor(
      "https://gis1.wim.usgs.gov/server/rest/services/GLCWRA/CRSRAZonalStats/GPServer/CRSRAZonalStats"
    );
    ...
    const GLRIWetlandsLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "CRSRA/MapServer",
      { id: "GLRIWetlands", visible: true, minScale: 100000, maxScale: 10000 }
    );
    ...
    const lakeLevelStationsLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "CRSRA/MapServer",
    ...
    var aerialsLayer = new FeatureLayer(mapServiceRoot + "CRSRA/MapServer/1", {
      id: "aerials",
      layerID: "aerials",
      visible: false,
      minScale: 100000,
    ...
    legendLayers.push({
      layer: aerialsLayer,
      title: "US Army Corps of Engineers Aerial Photos ",
    });
